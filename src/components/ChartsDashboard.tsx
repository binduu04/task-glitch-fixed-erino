...
export default function ChartsDashboard({ tasks }: Props) {
  ...
  // Fixed: Properly handle null/NaN ROI values in bucketing
  const roiBuckets = [
    { label: '<200', count: tasks.filter(t => t.roi != null && Number.isFinite(t.roi) && t.roi < 200).length },
    { label: '200-500', count: tasks.filter(t => t.roi != null && Number.isFinite(t.roi) && t.roi >= 200 && t.roi <= 500).length },
    { label: '>500', count: tasks.filter(t => t.roi != null && Number.isFinite(t.roi) && t.roi > 500).length },
    { label: 'N/A', count: tasks.filter(t => t.roi == null || !Number.isFinite(t.roi)).length },
  ];
