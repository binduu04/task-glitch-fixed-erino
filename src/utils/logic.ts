...
export function computeROI(revenue: number, timeTaken: number): number | null {
  ...
  if (timeTaken <= 0) return null; // Division by zero or negative time
  if (revenue < 0) return null; // Negative revenue doesn't make sense
...
export function sortTasks(tasks: ReadonlyArray<DerivedTask>): DerivedTask[] {
  return [...tasks].sort((a, b) => {
    ...
    // Stable tie-breaker: alphabetical by title (used because it is more user friendly alternative can be using creation timestamp (older first))
